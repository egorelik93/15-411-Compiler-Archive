...
-- from @f@ to @m@.
--
-- We model this internally as if left-catch holds.
--
...
instance Functor f => Monad (Free f) where
  ...
  Pure a >>= f = f a
  Free m >>= f = Free ((>>= f) <$> m)
  Plus xs >>= f = msum $ withStrategy (parList rseq) $ (>>= f) <$> xs

instance Functor f => Alternative (Free f) where
  empty = Plus []
  {-# INLINE empty #-}
  Pure a  <|> _       = Pure a -- left catch law
  Plus [] <|> r       = r
  l       <|> Plus [] = Plus [l]
