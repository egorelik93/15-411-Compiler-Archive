          ...
          do
            ...
            ((Var (L, len) =: UnOp Nop n)
             <> (wrap $ If (BinOp Lt (ALoc L len) (AImm 0))
                 segFault mempty)
             <> (Var (Q, n') =: BinOp Mul (AImm size) (ALoc L len))
             <> (Var (Q, n') =: BinOp Add (AImm 8) (ALoc Q n')))
        else do
          n <- e'
          ((Var (L, len) =: UnOp Nop n)
           <> (Var (Q, n') =: BinOp Mul (AImm size) (ALoc L len)))
...
translateExpr safe (FM.Free (runEnvT -> (t, Get e f))) = do
  ...
  let StructType _ fs = typeOf e
      ...
      findOffset (VarDecl t' f' _ : fs)
        ...
        | otherwise = bytes (sizeOf t') + findOffset fs
      offset = findOffset fs
      segFault = (Var (Q, AReg 0) =: UnOp Nop (AImm 0))
                 <> (return $ ALoc Q (AAddr (ALoc Q (AReg 0))))
  result <- newALoc
  return $ do
    v <- e'
    ((Var (Q, result) =: UnOp Nop v)
     <>
     (if safe then
     ...
     case sizeOf t of
       ...
       s ->
         (Var (Q, result) =: UnOp Nop (ALoc s $ AOffset offset (ALoc Q result)))
         <> return (ALoc s result))
...
translate :: Bool -> Map Ident (C0Type Ident)
          ...
          -> SymbolGen ([AVal], Stmt (Size, ALoc) (Term AVal) AVal)
translate safe ts (params, stmt) = do
