...
instance Memory Expr where
  ...
  arg 5 = Loc $ Reg R8D
  arg 6 = Loc $ Reg R9D
  arg i = Loc $ DerefOffset (fromIntegral $ 8 * (i - 7)) (Loc $ Reg RSP)

calleeSave :: Expr -> Bool
calleeSave (Loc (Reg EBX)) = True
calleeSave (Loc (Reg EBP)) = True
calleeSave (Loc (Reg R12D)) = True
calleeSave (Loc (Reg R13D)) = True
calleeSave (Loc (Reg R14D)) = True
calleeSave (Loc (Reg R15D)) = True
calleeSave (Loc (Reg RSP)) = True
calleeSave _ = False
...
instance (Memory m, Ord m) => Instruction (Instr m) where
  ...
  used (CALL _ v) = Set.fromList [arg i | i <- [1 .. length v]]
  used _ = Set.empty
  ...
  defined (CALL _ _) = Set.fromList $ [reg x | x <- [2 .. 8]] ++ [reg 0, reg 13]
  defined _ = Set.empty
