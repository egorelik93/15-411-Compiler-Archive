...
indexStmt (FMP.Free (Declare (VarDecl t id _) block)) = do
  (ts, indexMap) <- get
  aLoc <- lift newALoc
...
indexStmt (FMP.Free (Assign (Pointer v) expr)) = do
  v' <- indexLValue v
...
indexStmt (FMP.Free (Assign (Index v e) expr)) = do
  v' <- indexLValue v
  e' <- indexExpr e
  base <- lift newALoc
  expr' <- indexExpr expr
...
indexStmt (FMP.Free (Assign (Field v f) expr)) = do
  v' <- indexLValue v
  expr' <- indexExpr expr
  base <- lift newALoc
  (ts, _) <- get
...
translateStmt :: Stmt ALoc (TExpr AVal) (TExpr AVal)
              -> SymbolGen (Stmt ALoc (Term AVal) AVal)
...
translateStmt (FMP.Free (Assign (Var v) e)) = do
  s <- translateUpToOp e
  return $ do
    e' <- s
    Var v =: e'
translateStmt (FMP.Free (Assign (Pointer (Var v)) e)) = do
  s <- translateUpToOp e
  return $ do
    e' <- s
    Var (AAddr (ALoc Q v)) =: e'
translateStmt (FMP.Free (Assign (Index (Var v) i) e)) = do
  si <- translateExpr i
  se <- translateUpToOp e
  addr <- newALoc
  let segFault =
        (Var (AReg 0) =: UnOp Nop (AImm 0))
        <> (return $ ALoc Q (AAddr (ALoc Q (AReg 0))))
  ...
  return $ do
    ...
    e' <- se
    (
      (Var addr =: BinOp Sub (ALoc Q v) (AImm 8))
       <>
       (Var len =: UnOp Nop (ALoc L $ AAddr (ALoc Q addr)))
       <>
       wrap (If (BinOp GtE i' (AImm 0))
             (wrap $ If (BinOp Lt i' (ALoc Q len))
              (
                (Var addr =: BinOp Mul i' (AImm $ bytes size))
                <>
                (Var addr =: BinOp Add (ALoc Q addr) (ALoc Q v))
                <>
                (Var (AAddr (ALoc Q addr)) =: e')
              )
              segFault
             )
             segFault)
      )
translateStmt (FMP.Free (If e thn els)) = do
  e' <- translateUpToOp e
  ...
  case e of
    ...
    FM.Free (snd . runEnvT -> e) -> do
      ...
      return $ do
        ...
        wrap $ If e' thn' els'
-}
translateStmt (FMP.Free (While e b)) = do
  e' <- translateUpToOp e
  b' <- translateStmt b
  return $ do
    t <- e'
...
translateExpr :: TExpr AVal
              -> SymbolGen (Stmt ALoc (Term AVal) AVal)
translateExpr (FM.Pure i) =
  case runEnv i of
    (t, AImm i) -> do
      store <- newALoc
      return $ do
    ...
    (t, ALoc s (AAddr v)) -> do
      pointer <- newALoc
      result <- newALoc
      e <- translateUpToOp (FM.Pure $ env (PointerType t) v)
      return $ do
        v <- e
        Var pointer =: v
        Var result =: Deref (ALoc Q pointer)
        return (ALoc s result)
    (_, ALoc s v) -> return $ FMP.Pure (ALoc s v)
translateExpr (FM.Free (runEnvT -> (t, Call f params))) = do
  ss <- mapM translateUpToOp params
  alocs <- mapM (\_ -> newALoc) ss
  result <- newALoc
  let indexedSS = zip alocs ss
      sizes = map (sizeOf . typeOf) params
      indexedSizes = zip alocs sizes
  return $ do
    foldMapDefault (\(a, s) -> do {v <- s; Var a =: v}) indexedSS
    <> (Var result =: Call f (map (\(l, s) -> ALoc s l) indexedSizes))
    <> (return $ ALoc (sizeOf t) result)
translateExpr (FM.Free (runEnvT -> (t, Cond e1 e2 e3))) = do
  s1 <- translateUpToOp e1
  s2 <- translateUpToOp e2
  s3 <- translateUpToOp e3
  let size = sizeOf t
  result <- newALoc
  return $ do
    v1 <- s1
...
translateExpr (FM.Free (runEnvT -> (t, BinOp op e1 e2))) = do
  s1 <- translateExpr e1
  s2 <- translateExpr e2
  result <- newALoc
  return $ do
    ...
    <> return (ALoc (sizeOf t) result)
translateExpr (FM.Free (runEnvT -> (t, UnOp op e))) = do
  s <- translateExpr e
  result <- newALoc
  return $ do
    v <- s
    ...
    <> return (ALoc (sizeOf t) result)
translateExpr (FM.Free (runEnvT -> (t, Deref e))) = do
  e' <- translateExpr e
  result <- newALoc
  pointer <- newALoc
  return $ do
    v <- e'
    (Var pointer =: UnOp Nop v)
    <> (case sizeOf t of
           Aggregate _ -> return $ ALoc Q pointer
           s -> (Var result =: UnOp Nop (ALoc s $ AAddr (ALoc Q pointer)))
                <> return (ALoc s result)
       )
translateExpr (FM.Free (runEnvT -> (_, Alloc t))) = do
  ...
  case size of
    ...
    _ -> translateExpr $ wrap $ EnvT t' $
         Call "calloc" [return $ env IntType $ AImm size,
                        return $ env IntType $ AImm 1]
translateExpr (FM.Free (runEnvT -> (_, AllocArray t e))) = do
  let size = bytes $ sizeOf t
  e' <- translateExpr e
  n' <- newALoc
  a' <- newALoc
  len <- newALoc
  array <- newALoc
  s <- translateExpr $ wrap $ EnvT (ArrayType t) $
       Call "calloc" [return $ env IntType $ ALoc Q n',
                      return $ env IntType $ AImm 1]
  let segFault =
        (Var (AReg 0) =: UnOp Nop (AImm 0))
        <> (return $ ALoc Q (AAddr (ALoc Q (AReg 0))))
      evalLength = do
        ...
        ((Var len =: UnOp Nop n)
         <> (wrap $ If (BinOp Lt (ALoc L len) (AImm 0))
             segFault mempty)
         <> (Var n' =: BinOp Mul (AImm size) (ALoc L len))
         <> (Var n' =: BinOp Add (AImm 8) (ALoc Q n')))
      alloc = do
        a <- s
        ((Var array =: UnOp Nop a)
         <> (Var (AAddr (ALoc Q array)) =: UnOp Nop (ALoc L len))
         <> (Var a' =: BinOp Add (AImm 8) a)
         <> (return (ALoc Q a'))
          )
  return $ evalLength <> alloc
translateExpr (FM.Free (runEnvT -> (t, Elt e i))) = do
  e' <- translateExpr e
  i' <- translateExpr i
  size <- newALoc
  store <- newALoc
  ...
  return $ do
        a <- e'
        index <- i'
        ((Var store =: UnOp Nop a)
         <>
         (Var size =: UnOp Nop (ALoc L $ AOffset (-8) a'))
         <>
         (wrap $ If (BinOp GtE index (AImm 0))
           (wrap $ If (BinOp Lt index (ALoc L size))
            ((Var addr =: BinOp Mul index (AImm $ bytes (sizeOf t)))
             <> (Var addr =: BinOp Add (ALoc Q addr) a')
             <> (case sizeOf t of
...
translateExpr (FM.Free (runEnvT -> (t, Get e f))) = do
  e' <- translateExpr e
  let StructType _ fs = typeOf e
      ...
      findOffset (VarDecl t' f' _ : fs)
        ...
        | otherwise = bytes (sizeOf t') + findOffset fs
      offset = findOffset fs
      segFault = (Var (AReg 0) =: UnOp Nop (AImm 0))
                 <> (return $ ALoc Q (AAddr (ALoc Q (AReg 0))))
  result <- newALoc
  return $ do
    v <- e'
    ((Var result =: UnOp Nop v)
     <>
     (wrap $ If (BinOp Eq (ALoc Q result) (AImm 0)) segFault mempty)
     <>
     case sizeOf t of
       Aggregate _ -> (Var result =: BinOp Add (AImm offset) (ALoc Q result))
                      <> return (ALoc Q result)
       s ->
         (Var result =: UnOp Nop (ALoc s $ AOffset offset (ALoc Q result)))
         <> return (ALoc s result))

translateUpToTerm :: TExpr AVal
                  -> SymbolGen (Stmt ALoc (Term AVal) (Term AVal))
translateUpToTerm (FM.Pure i) = return $ return $ UnOp Nop $ snd $ runEnv i
translateUpToTerm (FM.Free e) =
  mapM translateExpr (snd $ runEnvT e) >>= (return . sequence)

translateUpToOp :: TExpr AVal
                  -> SymbolGen (Stmt ALoc (Term AVal) (Term AVal))
translateUpToOp e@(FM.Pure i) = translateUpToTerm e
translateUpToOp e@(FM.Free t) =
  case snd $ runEnvT t of
    BinOp _ _ _ -> translateUpToTerm e
    UnOp Nop e' -> translateUpToOp e'
    UnOp _ _ -> translateUpToTerm e
    _ -> do
      e' <- translateExpr e
...
translate :: Map Ident (C0Type Ident)
          -> ([VarDecl Ident], TStmt Ident)
          -> SymbolGen ([AVal], Stmt ALoc (Term AVal) AVal)
translate ts (params, stmt) = do
  ...
  result <- evalStateT (indexStmt stmt) (ts, paramCtx)
            >>= translateStmt
  return (paramLocs, result)
