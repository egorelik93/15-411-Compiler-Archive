    ...
    where
      makeParser term ops
        = let (rassoc,lassoc,nassoc
              ...
              manyFuncR :: ParsecT s u m (a -> a) -> ParsecT s u m (a -> a)
              manyFuncR p = fmap (foldr (.) id) $ many p

              manyFuncL :: ParsecT s u m (a -> a) -> ParsecT s u m (a -> a)
              manyFuncL p = fmap (foldr (flip (.)) id) $ many p

              termP      = do{ pre  <- manyFuncR prefixOp
                             ; x    <- term
                             ; post <- manyFuncL postfixOp
                             ; return (pre (post x))
                             }
