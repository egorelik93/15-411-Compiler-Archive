...
argTable = [
  ...
  Option [] ["x86_64"] (NoArg (setBackend X86_64)) "Uses X86-64 backend.",
  Option [] ["llvm"] (NoArg (setBackend LLVM)) "Uses LLVM backend."
  ]
...
setBackend :: Backend -> Job -> Job
setBackend b j = j {jobBackend = b}
