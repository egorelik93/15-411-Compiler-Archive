...
parseProgram :: [Ident] -> FilePath -> ErrorT String IO ([Ident], Program)
parseProgram ts file = do
  code <- liftIOE $ BS.readFile file
  let c0' = do
        prog <- c0
        ts' <- getState
        return (ts', prog)
  case runParser c0' ts file code of
    Left e -> throwError (show e)
    Right a -> return a

-- Utility for adding source positions
withPos :: Parsec s [Ident] (SourcePos -> e) -> Parsec s [Ident] e
withPos p = do
  pos <- getPosition
  content <- p
  return $ content pos
...
binary :: String -> (a -> a -> SourcePos -> a) -> Assoc
       -> Operator ByteString [Ident] Identity a
...
prefix :: String -> (a -> SourcePos -> a) -> Operator ByteString [Ident] Identity a
prefix syntax ast = Prefix $ do
  pos <- getPosition
  op syntax
  return $ \e -> ast e pos

postfix :: String -> (a -> SourcePos -> a) -> Operator ByteString [Ident] Identity a
...
ternaryCondOp :: Operator ByteString [Ident] Identity Exp
ternaryCondOp =
  Infix (do
            pos <- getPosition
            op "?"
...
indexOp :: forall u. Parsec ByteString [Ident] (Exp -> Exp)
indexOp = do
  pos <- getPosition
  e2 <- inBrackets exp
...
fieldOp :: Show e => String -> (e -> Ident -> SourcePos -> e)
        -> Parsec ByteString [Ident] (e -> e)
fieldOp syntax ast = do
  pos <- getPosition
  op syntax
  f <- ident
  return $ \e -> ast e f pos
...
c0type = (
  ...
  $ choice [
    try $ do
       ...
       ts <- getState
       if id `elem` ts
         then return $ DefType id
         else parserZero,
    try $ IntType <$ symbol "int",
    try $ BoolType <$ symbol "bool",
    try $ VoidType <$ symbol "void",
    try $ Struct <$> (keyword "struct" *> ident)
    ]
  ) <?> "type"
...
control = choice [
  ...
  withPos $ Return <$> (keyword "return" *> optionMaybe exp) <* semi,
  withPos $ Assert <$> (keyword "assert" *> inParens exp) <* semi
  ]
